 ...
 * @LastEditors: lzw
 * @LastEditTime: 2021-12-13 20:11:14
 * @Description:
 */
// @ts-check
...
class FEPSTemplate extends DependencyTemplate {
  ...
  apply(dep, source, templateContext) {
    const module = dep.moduleProxy || dep.module; // templateContext.moduleGraph.getModule(dep)
    if (!module || !dep.ast) return;
...
class FEPSDependency extends webpack.Dependency {
   ...
   * @param {comm.Program} ast
   * @param {string} relativePath
   * @param {comm.FepsOptions} options
   */
  constructor(module, ast, relativePath, options) {
    super();
